...
-- -- Weapon Menu --
-- RegisterNetEvent('nova_personalmenu:Weapon_addAmmoToPedC', function(value, quantity)
...
-- Admin Menu --
RegisterNetEvent('nova_personalmenu:Admin_BringC', function(plyCoords)
    ...
    ruiDrawContent(drawContentOptions, function()
        ...
        elseif _type == 'loadout' then
            ...
            RageUI.Button(i18nU('loadout_givemun_button'), "", {RightBadge = RageUI.BadgeStyle.Ammo}, true, function(Hovered, Active, Selected)
                ...
                TriggerServerEvent('nova_personalmenu:Weapon_addAmmoToPedS', GetPlayerServerId(closestPlayer), itemSelected.name, quantity)
    ...
    for i = 1, #PersonalMenu.BillData do
        ...
        RageUI.Button(billData.label, nil, { RightLabel = ('$%s'):format(GroupDigits(billData.amount)) }, true, function(Hovered, Active, Selected)
            ...
            TriggerServerCallback('esx_billing:payBill', function()
                TriggerServerCallback('nova_personalmenu:Bill_getBills', function(bills) PersonalMenu.BillData = bills end)
            end, billData.id)
        end)
    end
end
...
RegisterCommand('+openpersonal', function()
    ...
    TriggerServerCallback('nova_personalmenu:Admin_getUsergroup', function(plyGroup)
        PlayerVars.group = plyGroup
    end)

    TriggerServerCallback('nova_personalmenu:Bill_getBills', function(bills)
        PersonalMenu.BillData = bills
    end)

    TriggerEvent('nova_personalmenu:menuOpening')
